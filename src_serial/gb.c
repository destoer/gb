...
void handle_connection(int argc, char *argv[], Cpu *cpu)
{
	if(argc == 3)
	{
		...
		if(0 == strcmp(argv[2],"-l"))
		{
			...
			puts("Listening for a new connection");
			int rc = listen(cpu->c.listenSocket, SOMAXCONN); // listen for a new client
			if(rc == SOCKET_ERROR)
			{
				printf("listen failed: %d!\n",WSAGetLastError());
				closesocket(cpu->c.listenSocket);
				WSACleanup();
				exit(1);
			}
				
			// accept a client connection
			cpu->c.clientSocket = accept(cpu->c.listenSocket,NULL,NULL);
			if(cpu->c.clientSocket == INVALID_SOCKET)
			{
