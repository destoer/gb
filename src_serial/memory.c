...
void write_io(Cpu *cpu,uint16_t address, int data)
{
	...
	{ 
		...
		case IO_SC:
		{
			...
			#ifdef SERIAL
			// if the serial transfer is not active
			if(!cpu->serial_active)
			{
				// init transfer with internal clock
				cpu->is_master = is_set(data,0); // master gameboy
				
				// 7 is the trigger bit
				if(cpu->is_connected && cpu->is_master && is_set(data,7))
				{
					// transfer is in progress
					if(cpu->is_cgb && is_set(data,1))
					{
						cpu->serial_cyc = 4;
					}
					...
					send_byte(&cpu->c,0xff, SEND_INIT_SLAVE);
				}				
			}
			#endif
			return;
