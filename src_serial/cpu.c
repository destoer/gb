...
#ifdef SERIAL
void tick_serial(Cpu *cpu, int cycles);
#endif
...
void cycle_tick(Cpu *cpu,int cycles)
{
	...
	#ifdef SERIAL
	// ignore double speed for now for serial
	tick_serial(cpu,cycles);
	#endif
...
#ifdef SERIAL
// serial is currently buggy
void tick_serial(Cpu *cpu, int cycles)
{
	
	// serial is still shifting bits out
	// and the socket is still connected
	if(cpu->serial_active && cpu->is_connected)
	{
		if(cpu->is_master)
		{
			cpu->serial_cyc -= cycles;
			if(cpu->serial_cyc <= 0)
			{
				// setup the counter for when the next bit is going out
				if(cpu->is_cgb && is_set(cpu->io[IO_SC],1))
				{
					cpu->serial_cyc = 4;
				}
				...
				// push the data out
				char data = is_set(cpu->io[IO_SB],7) ? 1 : 0;

				
				
				if(cpu->is_connected)
				{
					cpu->is_connected = send_byte(&cpu->c,data,SEND_DATA) == SOCKET_ERROR? false: true;
				}
				...
				cpu->is_connected = recv_byte(&cpu->c,&data) == SOCKET_ERROR? false: true;

				cpu->io[IO_SB] <<= 1;
				cpu->io[IO_SB] |= data;
				
	
				// decrement the bit check no more are to be sent
				if(!--cpu->serial_cnt)
				{
					#ifdef SERIAL_DEBUG
					printf("%02x ",cpu->io[IO_SB]);
					#endif
					cpu->serial_active = false;
					if(!cpu->is_connected)
					{
						cpu->io[IO_SB] = 0xff;
					}
					deset_bit(cpu->io[IO_SC],7); // deset bit 7 to say tranfser has ended
		...
		// check if the linked gameboy 
		// (master) has sent any data in
		else
		{
			
			// if no data is waiting dont do anything
			if(!peek_data(&cpu->c))
			{
				return;
			}
			...
			char datarecv;
			char datasend = is_set(cpu->io[IO_SB],7) ? 1 : 0;
			
			if(cpu->is_connected)
			{			
				cpu->is_connected = recv_byte(&cpu->c,&datarecv) == SOCKET_ERROR? false: true;
			}
			...
			cpu->is_connected = send_byte(&cpu->c,datasend,SEND_DATA) == SOCKET_ERROR? false: true;

			
			cpu->io[IO_SB] <<= 1;
			cpu->io[IO_SB] |= datarecv;
	...
	else if(cpu->is_connected)
	{
		
		if(peek_data(&cpu->c)) // test if the other connected gameboy has started a xfer
		{
			char data;
			data = recv_byte(&cpu->c,&data);
			if(data == SEND_INIT_SLAVE)
			{
				
				// set the cycle time for pushing the 1st bit out / in
				if(cpu->is_cgb && is_set(cpu->io[IO_SC],1))
				{
					cpu->serial_cyc = 4;
				}
				...
				cpu->serial_cnt = 8;
				cpu->serial_active = true;
				cpu->is_master = false;
			}
			
			else if(data == SOCKET_ERROR)
			{
				cpu->is_connected = false;
				cpu->serial_active = false;
			}
