...
void push_pixel(Cpu *cpu) 
{
	...
	int col_num = cpu->ppu_fifo[0]; // save the pixel we will shift
	...
	int col = get_colour(cpu,col_num,0xff47); 
	int red = 0;
	int green = 0;
	int blue = 0;
	...
	cpu->screenp[scanline][cpu->x_cord] = col_num;
...
// for simplicity we will ignore the window for now
// and sprites 
...
void tick_fetcher(Cpu *cpu) {
	...
	// here should be our window check
	...
	if(cpu->tile_ready && cpu->pixel_count <= 8)
	{
		...
		// overlay with a sprite if at this posistion 
		
		// scroll thru our 10 sprites (may need a max sprite count)
		// if it triggers on this x 
		// fetch the one for this scanline by advancing the fetcher 
		// and then overlay it with the fifo

		
		else
		{
			memcpy(&cpu->ppu_fifo[cpu->pixel_count], cpu->fetcher_tile,8);
			cpu->tile_ready = false;
			cpu->pixel_count += 8;
		}
...
// needs window code removed plus scroll handling must be changed..
