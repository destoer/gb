...
// mario golf dont make to the only for gbc screen
...
// the actual memcpy for saving appears to be called from <-- memcpy at 311a
// 4db3:rom5 so step execution from here 
// 5178

//4caa

//4ccb and game is done saving just before the game locks up 

//4aec
																 // may not contribute to crash but needs to be investigated
// 1aaa <--- dies at the rst 08 and never returns called at 1aa4 <--- appears interrupt not occurring despite having the same register values?
